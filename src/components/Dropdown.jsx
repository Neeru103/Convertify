...
const CuurencyDropdown = ({
    ...
    return (
        <div>
            ...
            <label htmlFor={title} className='text-sm font-medium text-gray-700 block'>{title}</label>
            ...
            focus:outline-none focus:ring-2 focus:ring-indigo-500'>
                    {favorites.map(currency => {
                        ...
                        return <option className='bg-orange-200' value={currency} key={currency}>{currency}</option>
                    })}
                    <hr />
                    {currencies
